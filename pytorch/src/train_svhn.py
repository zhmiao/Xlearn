...
def transfer_classification(config, src):
    ...
    # full_net = network.network_dict['DTN_full']().cuda()
    # # pdb.set_trace()
    # full_net.load('./DTN_net_svhn_bal.pth')
    ...
    print('Loading ./DTN_net_svhn_bal.pth')
    weights = torch.load('./DTN_net_svhn_bal.pth')
    ...
    classifier_layer.weight.data = weights['classifier.2.weight'].data
    classifier_layer.bias.data = weights['classifier.2.bias'].data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', type=str, nargs='?', default='svhn_bal', help="source data")
